...
public class CaptchaModuleConfig {
    ...
    private String messageType = CaptchaModuleConfig.MESSAGE_TYPE_TEXT;

    private String messageValue = "You failed the jcaptcha test";

    private String messageKey = "jcaptcha_fail";

    private String idType = CaptchaModuleConfig.ID_SESSION;
    ...
    public void validate() {
        ...
        //verify values
        if (!(CaptchaModuleConfig.MESSAGE_TYPE_TEXT.equals(messageType) || CaptchaModuleConfig.MESSAGE_TYPE_BUNDLE.equals(messageType)))
            throw new com.octo.captcha.service.CaptchaServiceException("messageType can " +
                    "only be set to '" + CaptchaModuleConfig.MESSAGE_TYPE_TEXT + "' or '" + CaptchaModuleConfig.MESSAGE_TYPE_BUNDLE + "'");

        if (!(CaptchaModuleConfig.ID_SESSION.equals(idType) || CaptchaModuleConfig.ID_GENERATED.equals(idType)))
            throw new com.octo.captcha.service.CaptchaServiceException("idType can " +
                    "only be set to '" + CaptchaModuleConfig.ID_SESSION + "' or '" + CaptchaModuleConfig.ID_GENERATED + "'");
        ...
        if ((idType.equals(CaptchaModuleConfig.ID_GENERATED)) && (idKey == null || "".equals(idKey)))
            throw new com.octo.captcha.service.CaptchaServiceException("idKey cannot be null or empty when id is generated (ie idType='" + CaptchaModuleConfig.ID_GENERATED + "'");

        //if message is in a bundle, try to load it
        if (this.messageType.equals(CaptchaModuleConfig.MESSAGE_TYPE_BUNDLE)) {
